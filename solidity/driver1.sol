...
uint moneyInPool = 0;
  ...
  function init_members(uint _aadhaar, uint init_amount) {
    usr_aadhar[_aadhaar]=msg.sender;
    if(init_member_counter <5){
      link[msg.sender]=member(now,4,msg.sender,_aadhaar,init_amount,0x1,0x2,0x3,0x4);
      init_member_counter++;
    }
    else{
      throw;
  ...
  //Money a member has
  function moneyHeHas() constant returns (uint){

      return link[msg.sender].moneyHeHas;
  }

  function addMoneyToAccount(uint amt){
      link[msg.sender].moneyHeHas = link[msg.sender].moneyHeHas + amt;
  }
  ...
  function add_Member(address _req_member,uint __aadhaar) check_eligibility_of_payments(msg.sender) {
    ...
    {usr_aadhar[__aadhaar]=_req_member;
      var1=msg.sender;
      link[_req_member]=member(now,count,_req_member,__aadhaar,0,var1,0,0,0);
    }
    else if (count==2)
    {
  ...
  function pool(uint __amount) payable  {
    ...
    if(link[msg.sender].moneyHeHas>__amount)
    {
        moneyInPool = moneyInPool + __amount;
        link[msg.sender].moneyHeHas = link[msg.sender].moneyHeHas- __amount;
  ...
  modifier every_3_months {
    ...
    uint months=(now-TimeStart)/(1); //1 seconds
    if(months%30==0)
    {
        _;
    }
  ...
  function pay_loan() {
      ...
      if(moneyInPool>amounts[w]){
            link[temp_address].moneyHeHas = link[temp_address].moneyHeHas + amounts[w];
            moneyInPool = moneyInPool - amounts[w];
